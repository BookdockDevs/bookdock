...
The repository publishes Docker Hub images when a `v*` tag is pushed. To enable that workflow, add `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN` as GitHub Actions secrets, create a public Docker Hub repository named `bookdock`, and push a release tag. For example, `v0.3.3` publishes `<dockerhub-username>/bookdock:0.3.3`, `<dockerhub-username>/bookdock:0.3`, and `<dockerhub-username>/bookdock:latest`. Until an image is published, use the source-build command above.
