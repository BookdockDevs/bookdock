...
- The shipped `docker-compose.yml` sets no environment variables on purpose: `NODE_ENV=production` and `DATA_DIR=/data` are baked into the image, and every other variable has a working default. The full table lives in [`docs/architecture.md`](docs/architecture.md) (§ Config & Env).
...
The repository publishes Docker Hub images when a `v*` tag is pushed. To enable that workflow, add `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN` as GitHub Actions secrets, create a public Docker Hub repository named `bookdock`, and push a release tag. For example, `v0.3.4` publishes `<dockerhub-username>/bookdock:0.3.4`, `<dockerhub-username>/bookdock:0.3`, and `<dockerhub-username>/bookdock:latest`. Until an image is published, use the source-build command above.
