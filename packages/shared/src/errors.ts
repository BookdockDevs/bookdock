export const ErrorCode = {
  BOOK_NOT_FOUND: 'BOOK_NOT_FOUND',
  LIBRARY_NOT_FOUND: 'LIBRARY_NOT_FOUND',
  LIBRARY_VERSION_NOT_FOUND: 'LIBRARY_VERSION_NOT_FOUND',
  SHELF_NOT_FOUND: 'SHELF_NOT_FOUND',
  SHELF_NAME_TAKEN: 'SHELF_NAME_TAKEN',
  TAG_NOT_FOUND: 'TAG_NOT_FOUND',
  TAG_NAME_TAKEN: 'TAG_NAME_TAKEN',
  USER_NOT_FOUND: 'USER_NOT_FOUND',
  ANNOTATION_NOT_FOUND: 'ANNOTATION_NOT_FOUND',
  SESSION_NOT_FOUND: 'SESSION_NOT_FOUND',
  FONT_NOT_FOUND: 'FONT_NOT_FOUND',
  AVATAR_NOT_FOUND: 'AVATAR_NOT_FOUND',
  REPLACEMENT_NOT_FOUND: 'REPLACEMENT_NOT_FOUND',
  TOC_RULE_NOT_FOUND: 'TOC_RULE_NOT_FOUND',
  TTS_NOT_CONFIGURED: 'TTS_NOT_CONFIGURED',
  TTS_SERVICE_NOT_FOUND: 'TTS_SERVICE_NOT_FOUND',
  TTS_PROVIDER_NOT_SUPPORTED: 'TTS_PROVIDER_NOT_SUPPORTED',
  TTS_PROVIDER_ERROR: 'TTS_PROVIDER_ERROR',
  TTS_NOT_ALLOWED: 'TTS_NOT_ALLOWED',
  AI_NOT_CONFIGURED: 'AI_NOT_CONFIGURED',
  AI_NOT_ALLOWED: 'AI_NOT_ALLOWED',
  AI_PROFILE_NOT_FOUND: 'AI_PROFILE_NOT_FOUND',
  AI_THREAD_NOT_FOUND: 'AI_THREAD_NOT_FOUND',
  AI_MESSAGE_NOT_FOUND: 'AI_MESSAGE_NOT_FOUND',
  AI_RUN_NOT_FOUND: 'AI_RUN_NOT_FOUND',
  AI_RUN_CONFLICT: 'AI_RUN_CONFLICT',
  AI_INDEX_FAILED: 'AI_INDEX_FAILED',
  AI_BUSY: 'AI_BUSY',
  AI_RATE_LIMITED: 'AI_RATE_LIMITED',
  AI_TIMEOUT: 'AI_TIMEOUT',
  AI_PROVIDER_ERROR: 'AI_PROVIDER_ERROR',
  BOOK_FILE_MISSING: 'BOOK_FILE_MISSING',
  TOKEN_NOT_FOUND: 'TOKEN_NOT_FOUND',
  UNAUTHORIZED: 'UNAUTHORIZED',
  FORBIDDEN: 'FORBIDDEN',
  USERNAME_TAKEN: 'USERNAME_TAKEN',
  REGISTRATION_DISABLED: 'REGISTRATION_DISABLED',
  ACCOUNT_DISABLED: 'ACCOUNT_DISABLED',
  AUTH_RATE_LIMITED: 'AUTH_RATE_LIMITED',
  CANNOT_MODIFY_SELF: 'CANNOT_MODIFY_SELF',
  LAST_OWNER: 'LAST_OWNER',
  CANNOT_MODIFY_GUEST: 'CANNOT_MODIFY_GUEST',
  VALIDATION_ERROR: 'VALIDATION_ERROR',
  UPLOAD_TOO_LARGE: 'UPLOAD_TOO_LARGE',
  UNSUPPORTED_FORMAT: 'UNSUPPORTED_FORMAT',
  TRASH_DISABLED: 'TRASH_DISABLED',
  SNAPSHOT_NOT_FOUND: 'SNAPSHOT_NOT_FOUND',
  SNAPSHOT_CREATE_FAILED: 'SNAPSHOT_CREATE_FAILED',
  UPDATE_IN_PROGRESS: 'UPDATE_IN_PROGRESS',
  UPDATE_NOT_AVAILABLE: 'UPDATE_NOT_AVAILABLE',
  UPDATE_NOT_LAUNCHED: 'UPDATE_NOT_LAUNCHED',
  UPDATE_FAILED: 'UPDATE_FAILED',
  INTERNAL_ERROR: 'INTERNAL_ERROR',
} as const

export type ErrorCode = (typeof ErrorCode)[keyof typeof ErrorCode]

export const ErrorHttpStatus: Record<ErrorCode, number> = {
  BOOK_NOT_FOUND: 404,
  LIBRARY_NOT_FOUND: 404,
  LIBRARY_VERSION_NOT_FOUND: 404,
  SHELF_NOT_FOUND: 404,
  SHELF_NAME_TAKEN: 409,
  TAG_NOT_FOUND: 404,
  TAG_NAME_TAKEN: 409,
  USER_NOT_FOUND: 404,
  ANNOTATION_NOT_FOUND: 404,
  SESSION_NOT_FOUND: 404,
  FONT_NOT_FOUND: 404,
  AVATAR_NOT_FOUND: 404,
  REPLACEMENT_NOT_FOUND: 404,
  TOC_RULE_NOT_FOUND: 404,
  TTS_NOT_CONFIGURED: 400,
  TTS_SERVICE_NOT_FOUND: 404,
  TTS_PROVIDER_NOT_SUPPORTED: 400,
  TTS_PROVIDER_ERROR: 502,
  TTS_NOT_ALLOWED: 403,
  AI_NOT_CONFIGURED: 503,
  AI_NOT_ALLOWED: 403,
  AI_PROFILE_NOT_FOUND: 404,
  AI_THREAD_NOT_FOUND: 404,
  AI_MESSAGE_NOT_FOUND: 404,
  AI_RUN_NOT_FOUND: 404,
  AI_RUN_CONFLICT: 409,
  AI_INDEX_FAILED: 503,
  AI_BUSY: 429,
  AI_RATE_LIMITED: 429,
  AI_TIMEOUT: 504,
  AI_PROVIDER_ERROR: 502,
  BOOK_FILE_MISSING: 404,
  TOKEN_NOT_FOUND: 404,
  UNAUTHORIZED: 401,
  FORBIDDEN: 403,
  USERNAME_TAKEN: 409,
  REGISTRATION_DISABLED: 403,
  ACCOUNT_DISABLED: 403,
  AUTH_RATE_LIMITED: 429,
  CANNOT_MODIFY_SELF: 400,
  LAST_OWNER: 400,
  CANNOT_MODIFY_GUEST: 400,
  VALIDATION_ERROR: 400,
  UPLOAD_TOO_LARGE: 413,
  UNSUPPORTED_FORMAT: 415,
  TRASH_DISABLED: 403,
  SNAPSHOT_NOT_FOUND: 404,
  SNAPSHOT_CREATE_FAILED: 500,
  UPDATE_IN_PROGRESS: 409,
  UPDATE_NOT_AVAILABLE: 409,
  UPDATE_NOT_LAUNCHED: 409,
  UPDATE_FAILED: 500,
  INTERNAL_ERROR: 500,
}
