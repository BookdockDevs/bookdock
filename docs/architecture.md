...
> Last updated: 2026-09-16 · This document is the authoritative architecture blueprint; the code follows it. When an architectural decision changes, update this document first, then change the code.
...
- `Settings(id, userId, key, value)` — the `ui` value may include per-user `fontPreferences` keyed by stable system/builtin/uploaded font ids and a `fontOrder` list of those ids; these preferences control display name, visibility, and display order without turning non-file fonts into database rows. `GET /settings` also injects a read-only instance-level `uploadMaxBytes` (from the `UPLOAD_MAX_BYTES` env) for client-side display and pre-check; `settingsUpdateSchema` strips it from PUT bodies so it is never persisted. A user-configurable limit is a near-term candidate (see `docs/local/plan.md`).
...
| `settings` | id, userId FK, key, value (json) | unique (userId, key); keys: `ui` (reader/UI prefs plus `fontPreferences` display-name/enabled overrides and `fontOrder`), `trash` (`{ autoCleanDays, enabled? }`; omitted `enabled` = on, PUT merges partial updates and purges the trash when disabling), `ai` (encrypted user AI provider configuration) |
...
- Inline EPUB audio/video stays native and non-blocking: the vendored core prioritizes instant text rendering over heavy media decompression. During chapter XHTML preparation, heavy video and audio sources are deferred into `data-bd-deferred-src`, allowing chapter text to render without waiting for multi-megabyte binary decompression from the EPUB archive. Once rendered, `FoliateReader` normalizes media containers into fixed 16:9 `.bd-video-wrapper` boxes with subtle loading spinner feedback and resolves the Blob URLs asynchronously via `section.loadHref()`, guaranteeing zero layout shift and unchanged pagination. The vendored core preserves controls, source, track, and poster semantics, suppresses reader page-turn handling when a media element is clicked, and reports a media load error to the host without interrupting text rendering. The Loader owns static and dynamic media Blob URLs and releases them with the section/frame lifecycle; React does not create a second media cache or custom player for this baseline.
...
- Reader positions have separate persistence semantics: the progress `cfi` may remain a layout-oriented `chapter:index:fraction` coordinate in scrolled mode so resume and coverage restore the viewport proportion, while `ReaderLocation.anchorCfi` preserves the engine's raw visible-content EPUB CFI for new bookmarks. New bookmark `cfiRange`/`cfiAnchor` values use that content anchor and their excerpt starts at the anchor's range start; existing fraction-based bookmarks are supported as a legacy fallback and are not migrated.
