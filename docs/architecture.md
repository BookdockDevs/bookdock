...
- **The update state machine** (`POST /api/v1/system/update`, watched by `GET /api/v1/system/update/status`; owner-only, single in-flight reservation keyed by a client progress id, and refused outright when `BOOKDOCK_LAUNCHER_NONCE` is absent, i.e. not under a launcher): synchronously reserve before any asynchronous preflight; same-id retries join that start, different ids receive 409, and failed preflight releases the reservation. Then check → automatic snapshot → download → verify → extract → atomic promote → `pending` marker → server exits `75` (`EX_TEMPFAIL`) → launcher re-resolves and health-gates the child on `/api/v1/health` plus the version fingerprint `GET /api/v1/internal/version` (mounted above the auth guard, answered only with the per-boot `x-bookdock-launcher-nonce`; dead without `BOOKDOCK_LAUNCHER_NONCE`, so a no-op in dev) → commit (move `current`, keep `previous`) or roll back (restore the snapshot database and drop `pending`; `current` was never moved, so no pointer rewrite is owed) → report in the panel. The status answer is derived from disk wherever possible, so one poller spans the restart it watches. Snapshots are `DATA_DIR/snapshots/<version>-<ts>/bookdock.db` produced by the better-sqlite3 Online Backup API via `POST/GET/DELETE /api/v1/system/snapshots`; retain the newest completed snapshot plus any snapshot reserved or referenced by the active update, and reject deletion of that rollback target until the update resolves. This bounds the normal cache to one full database copy while preserving the active rollback point; older snapshots are not a supported multi-version restore feature. Download bytes and expanded zip entries are streamed through their respective size caps; the package checksum is computed while downloading and verified before extraction. The user-facing flow is a single "update" button with progress labels for snapshot, download, verification, extraction, promotion, and restart.
