...
| `library_migration_log` | id, batch, status, details (json), startedAt, finishedAt? | Phase 0/2 migration ledger: one row per batch, completed batches refuse reruns |
...
- The release migration path is append-only and fully represented by Drizzle metadata: a `v0.2.0` database applies the forward migrations through `0009_reading_entities.sql` (`0001_release_0_2_1.sql`, text replacements, Legado access keys, scoped access tokens, annotation chapter hrefs, shelf/tag sorting metadata, then the `0008_library_foundation.sql` library/book/blob/identity/session structure and the `0009_reading_entities.sql` reading tables), while fresh databases apply the baseline followed by the same chain. Startup also retains narrow idempotent guards for local databases whose consolidated migration ledger is ahead of the physical schema; they repair only known columns, indexes, and text-replacement schema details, never rebuilding or deleting user data.
...
- **Persistence**: all mutable state lives under `DATA_DIR`, including SQLite, uploaded files, generated content, progress, `.jwt-secret`, and — once an in-app update has run — the managed application code itself (`releases/`) and pre-update snapshots (`snapshots/`). A cold backup copies the complete directory while the container is stopped; restore replaces the complete directory before startup. The `.jwt-secret` file must be preserved to keep existing sessions valid.
