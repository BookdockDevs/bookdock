...
- **Reader dialogs**: The reader supplies its occupied left width to dialogs through React context, including dialogs portaled to `document.body`. On desktop, reader-dialog backdrops cover only the visible reading region and center their panels there; the sidebar stays visible and interactive without dismissing the dialog. Touch layouts keep their existing centered or bottom-sheet placement. Dialogs outside the reader retain viewport-wide backdrops and centering.
- **Core Design Goals**:
...
- `Annotation(id, userId, bookId, cfiRange, cfiAnchor?, type, color, style, text, note?, chapter?, chapterHref?, createdAt, updatedAt)` — `chapterHref` preserves the TOC item's stable href so annotations remain distinguishable when chapter labels repeat.
...
Chapter-title layout is a separate reader preference: alignment (start,
center, or end), size relative to body text, and spacing above and below the
title. The spacing uses body-text units, so changing title size alone does not
change either gap. These preferences apply to generated TXT chapter headings
in both reading modes. In EPUB they apply only with `overrideBookLayout` enabled
and only to a heading that can be identified from the book's TOC; unmatched
content keeps its authored style. They do not change in-body subheadings or
page geometry. Existing reading profiles without these fields resolve to the
generated TXT heading's previous appearance.
...
apps/server/src/
  ...
  modules/
    auth.routes.ts          # server-side sessions, instance settings, /setup, /login, /logout, /register, /password, /username
...
**Current tables** (42, including the FTS virtual table):

| Table | Key columns | Notes |
|---|---|---|
| `users` | id (text PK), username (unique), usernameNormalized? (unique, nullable until backfilled), bio, passwordHash?, role, disabled, avatarKey?, createdAt, updatedAt? | usernameNormalized is trim/NFKC/case-folded, NULL during the compat window (SQLite UNIQUE ignores NULLs); bio plain text default ''; role: owner\|member\|guest; disabled → deny; avatarKey = content-hash blob key under `avatars/`, ref-checked before physical delete |
...
| `shelves` | id, userId FK, name, sortOrder, createdAt, updatedAt, pinned | name is unique per user after trimming; updatedAt is membership-change time maintained by SQLite triggers; pinned items lead every sidebar sort mode; deleting a shelf sets its books' shelfId to NULL (books become 未分类, never deleted) |
| `tags` | id, userId FK, name, sortOrder, createdAt, updatedAt, pinned | name is unique per user after trimming; sortOrder is rewritten from the submitted full tag id list; updatedAt is membership-change time maintained by SQLite triggers; pinned items lead every sidebar sort mode; the library sidebar and active library heading use the selected tag's name |
| `book_tags` | bookId FK (cascade), tagId FK (cascade) | composite PK, M2M |
| `settings` | id, userId FK, key, value (json) | unique (userId, key); keys: `ui` (reader/UI prefs plus `fontPreferences` display-name/enabled overrides and `fontOrder`), `trash` (`{ autoCleanDays, maxTrashBytes?, enabled? }`; omitted `enabled` = on, PUT merges partial updates and purges the trash when disabling), `library` (`{ normalizeTitle?, shelfSort?, tagSort?, bookSort?, view? }`; omitted values use defaults, and URL state takes precedence over server settings), `integrations` (`{ legado?: { enabled?, authMode?: 'login'|'accessKey' } }`; omitted `enabled` = off), `ai` (encrypted user AI provider configuration) |
...
| `annotations` | id, userId FK, bookId FK, cfiRange, cfiAnchor?, type, color, style, text, note?, chapter?, chapterHref?, createdAt, updatedAt, **deletedAt?** | unique (userId, bookId, cfiRange); soft delete |
...
| `libraries` | id, userId (= owner, tenant key), type (private\|shared), name, description, visibility (public\|password\|private)?, createdAt, updatedAt | one Private Library per real user (no membership rows); Shared Libraries carry an owner and optional memberships; `user_id` holds the owner so the per-user scoping rule needs no second column, exposed on the wire as `ownerUserId` |
...
| `book_states` | userId FK (cascade), bookVersionId FK (cascade), readStatus, percent, cfi?, chapter?, updatedAt | composite PK (userId, bookVersionId); intervals/speed samples stay in storage files under the same dimension |
...
- **Auth**: random server-side session tokens are stored only as hashes in SQLite and delivered via HttpOnly Cookie `bd_token` (SameSite=Strict, Path=/); Bearer header also accepts a session token. Sessions have a 30-day sliding expiry, refreshed inside seven days of expiry. Existing JWT cookies are not converted and require a new login. CSRF: cookie is same-site only + API is JSON-only.
- **Login protection**: failed credential attempts use a process-local 60-second sliding window keyed by client address + trimmed username. `AUTH_RPM` (default 5, max 120) returns `AUTH_RATE_LIMITED` (429) with `Retry-After`; validation errors, successful logins, and disabled-account responses are not counted. The window resets on process restart and is not shared across replicas. Password-setting endpoints (setup/register/new password/owner reset) enforce `AUTH_PASSWORD_MIN_LENGTH`–`AUTH_PASSWORD_MAX_LENGTH` (8–256, NIST-style: length + KDF, no composition rules or rotation); login and `oldPassword` accept any non-empty string so validation never leaks credential validity. Storage is scrypt with a per-password random salt and constant-time comparison.
...
- **Username uniqueness**: username inputs are sanitized at the zod validation layer (`sanitizeUsername` strips C0/C1 control, zero-width, and bidi-control characters, then trims), so stored usernames are display-safe. Uniqueness for register/rename/setup is checked against the normalized form (`normalizeUsername` = sanitized + NFKC + lowercase), so `Admin`, `admin`, and `Ａdmin` cannot coexist as spoofing handles; the byte-exact `users.username` UNIQUE constraint remains as the race backstop. The nanoid `id` is the authorization identity; the username is only a mutable login alias and display name.
...
- **Roles**: `owner` (instance admin), `member` (registered), `guest` (anonymous → default user). First boot must create owner via `/setup` (web guard redirects when `initialized=false`). The guest account is identified by role, lazily created on the first guest request with its username set to its own id (never logs in, hidden from user management, so the name only has to be collision-proof), hidden from user management (`listUsers` excludes it, `updateUser` rejects any patch on it), and managed solely through the `allowGuestAccess` switch. Guest sessions are temporary, anonymous, and read-only: the auth guard rejects every guest `POST`/`PUT`/`PATCH`/`DELETE`, while public login/logout/setup routes remain available. Guests may use local display and reader preferences and the browser-local `bd-guest-reading-progress` namespace, but must not persist shared-account settings or manage library-management settings, integrations, tokens, AI/TTS, account, or instance configuration; those surfaces require a signed-in user or the owner as appropriate. Guest direct downloads/exports, trash access, stats, annotations, and reading-record access are closed; the reader's file stream is only available through its explicit reader request. The public-library visibility model is intentionally deferred and is not implied by guest access.
...
|---|---|---|
| `/api/v1/health` | — | `GET /` |
| `/api/v1/system` | system | `GET /info` `GET /update-check` (behind the auth guard; build/version links plus a cached GitHub latest-release check), `GET /update/status` `POST /update` (owner-only, in-app update per ADR-25) |
...
**Progress storage**: signed-in users persist per-book JSON files at `progress/{bookId}.json`, shape `{ cfi?, chapter?, chapterIndex?, percent, fraction, intervals, updatedAt }`. `chapterIndex` is the zero-based chapter position from the current book TOC and is persisted for server-side consumers such as AI spoiler bounds; AI applies this boundary at chapter granularity, so the whole current chapter is available and later chapters are not. `fraction` is the foliate book-wide position (0–1); `intervals` is the merged union of `[start, end]` fraction ranges the user has actually read — the web reader closes the current segment when a relocate jump exceeds `JUMP_THRESHOLD` (0.02) and reports `segmentStartFraction` on PUT. GET additionally returns `readFraction` (total union length, computed server-side). Guests never read or write this server progress; their reader position uses the browser-local `bd-guest-reading-progress` namespace instead.
...
- **Library pagination**: the home library keeps its one-based page in the URL, fetches configurable page batches (24/48/96 books, default 24) through `useBooks`, and renders the current page in ordinary grid/list containers. Changing a library filter resets to page one; page navigation replaces the URL state and clears page-scoped selection. The numbered page capsule supports adjacent navigation, faster travel across page ranges, and direct page entry from the active page or `g` shortcut; page size is a device-local display preference.
...
- A user TOC click records the selected href immediately, even before the renderer is available. Once available, the renderer navigates to that target ahead of saved CFI, saved fraction, or default first-chapter startup positioning. The clicked entry stays highlighted while awaiting relocation and remains selected on navigation failure; failure and retry feedback stays in the reader body. Each sidebar panel keeps a per-book scroll position, and chapter-grouped notes wait for the reader TOC before rendering.
...
- Paginated spine sections may share a spread when a section ends before its final column; chapter boundaries do not insert blank columns. On a spread containing two sections, the later visible section owns the current position, chapter label, and subsequent mode switch. Switching from paginated to non-continuous scrolled flow discards adjacent section views before rendering, so the viewport contains only that current section.
...
- **docker-compose.yml**: minimal on purpose — defaults to the published `loofffff/bookdock:latest` stable image while retaining the build context for local builds; set `BOOKDOCK_IMAGE` to pin or override the image. It also defines the port mapping, `./data:/data` mount, `restart: unless-stopped`, and no `environment:` block (runtime env comes from image `ENV`s + schema defaults; overrides are added locally). The mutable `latest` tag advances on stable releases only; a version tag remains available for deployments that need a fixed release.
...
- **Persistence**: all mutable state lives under `DATA_DIR`, including SQLite, uploaded files, generated content, progress, `.jwt-secret`, and — once an in-app update has run — the managed application code itself (`releases/`) and pre-update snapshots (`snapshots/`). A cold backup copies the complete directory while the container is stopped; restore replaces the complete directory before startup. Preserve `.jwt-secret` because AI, TTS, and Legado encrypted settings still derive keys from it; login sessions live in SQLite.

### 9.1 In-app self-update (ADR-25)

- **Code truth is the volume, not the image.** The image ships `runtime + launcher + factory app`; PID 1 is a deliberately minimal `node /app/launcher.mjs` (no dependencies, never updated from the panel) that resolves `DATA_DIR/releases/current` (an atomically renamed pointer file) and falls back to the image's `/app` — overridable via `BOOKDOCK_FACTORY_ROOT` — when no managed release exists. If the image version is newer than the volume's current release, the image wins ("official pull always wins", strictly newer comparison), so `docker compose pull` remains a valid upgrade path forever.
- **The volume holds at most two releases** (`current` + `previous`); the rollback path must never depend on the network or on an upstream artifact still existing. The factory copy in the image is the first update's rollback target.
- **Per-release CI artifacts** accompany the image, exported from a dedicated `bundle` Dockerfile stage so image and artifact are the same tree: `bookdock-<version>-linux-x64-musl.zip` (server bundle + web `dist` + production `node_modules` with the prebuilt `better-sqlite3`, plus the `release.json` the launcher reads), a detached `<file>.sha256`, and that `release.json` also uploaded standalone as the release asset the panel reads (`version`, `nodeMajor`, `libc`, `createdAt`). Zip is deliberate: the extracting side is the running server, which already depends on `jszip` and has no tar reader or build toolchain. The artifact carries no launcher. The updater verifies sha256 before extraction, extracts zip-slip-safe and size-capped into `releases/<version>.work/`, then renames the staged `app/` tree into place and removes the work directory. A non-matching `nodeMajor`/`libc` is refused before any byte is written, with a message pointing non-alpine deployments at `docker compose pull`.
- **The update state machine** (`POST /api/v1/system/update`, watched by owner-only `GET /api/v1/system/update/status`, cancelled with owner-only `DELETE /api/v1/system/update/:progressId`; refused outright when `BOOKDOCK_LAUNCHER_NONCE` is absent): single-flight reservation keyed by the client progress id, with idempotent retries and conflicts for another active task. A bounded `releases/update-state.json` record is the authoritative resumable task/last-result snapshot; it stores lifecycle timestamps, current phase/action, phase duration, safe error code/message and request id, plus phase-specific counters, never raw logs, credentials, or full request URLs. Each independently reported phase is real work: snapshot pages/total pages, download connection/response/receiving with received bytes and optional content length, extraction file/byte counters with percentage only when a reliable total exists, and explicit checksum/promotion/launcher-check actions. No weighted aggregate percentage is used. Status carries an `active`/`terminal` outcome discriminator so transport/query errors remain distinct from completed results. The running process preserves the latest terminal result across a page reopen; the launcher updates that same record to `succeeded` or `rolled-back` after its version health gate and snapshot recovery.

  The order is check → automatic snapshot → download → verify → extract → atomic promote → `pending` marker → server exits `75` (`EX_TEMPFAIL`) → launcher re-resolves and health-gates the child on `/api/v1/health` plus the version fingerprint `GET /api/v1/internal/version` (mounted above auth, answered only with the per-boot `x-bookdock-launcher-nonce`) → commit (move `current`, keep `previous`) or roll back (restore the snapshot database and report the old running version). Cancellation is accepted only before promotion: abort the active fetch/stream or snapshot operation where supported, wait for exclusive cleanup of `.work` and snapshot retention, then record `cancelled`; once promotion starts cancellation is rejected and launcher health-gate/automatic rollback remains authoritative. Snapshot page counts are surfaced by the backup callback. Download has bounded connection-idle and overall timeouts, package size caps, and progress while streaming. Snapshots remain `DATA_DIR/snapshots/<version>-<ts>/bookdock.db`; retain the newest completed snapshot plus any active rollback target and reject its deletion. The in-panel updater changes managed application files only; Compose image pulls retain their independent “newer image wins” behavior. The launcher itself is still updated only through images.
- **Bootstrap**: the first release containing the launcher still arrives via `docker compose pull`; its release notes say so, and the full chain (happy path + failed-update rollback) must pass a Docker integration test with two synthetic versions before that release ships. The launcher itself is only ever updated through images and has a frozen scope.
- **Initialization**: database schema migrations, narrow idempotent schema-compatibility guards, derived-artifact migrations, and the private-library data backfill run during startup (backfill before book-id retarget; blocked databases refuse to boot instead of serving empty libraries). The 0.2.1 release upgrades an existing 0.2.0 `DATA_DIR` through forward migrations; existing 0.1.0 data is not an upgrade target. The 0.3.7 upgrade path (0.3.5-era databases through the backfill and retarget) was validated on an isolated copy; Docker and in-panel upgrades share the same boot path. TXT books are stored as generated EPUB artifacts, so each artifact format change must be versioned in `books.meta` and upgraded atomically before the server starts serving book files. Back up any local data before upgrading, start the new image, and verify `/api/v1/health` plus the web setup/login flow.
- **Registry publishing**: `.github/workflows/release.yml` publishes `v*` git tags to Docker Hub as a version tag and a major/minor tag; stable tags also advance `latest`. It requires the repository secrets `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN`; no registry credentials are stored in the repository.
...
| ADR-25 | in-container self-update via launcher and volume-held releases | `docs/local/adr/0025-launcher-volume-releases-self-update.md` |
