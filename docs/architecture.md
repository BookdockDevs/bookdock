...
| `ai_threads` | id, userId FK, bookId FK (cascade), title, settings (json)?, createdAt, updatedAt | per-user, per-book persisted AI conversation; settings store the thread's reading-scope and read-tool defaults; title is local metadata and contains no generated provider content |
