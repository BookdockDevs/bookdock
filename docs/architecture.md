...
- **Library pagination**: the home library keeps its one-based page in the URL, fetches configurable page batches (24/48/96 books, default 24) through `useBooks`, and renders the current page in ordinary grid/list containers. Changing a library filter resets to page one; page navigation replaces the URL state and clears page-scoped selection.
