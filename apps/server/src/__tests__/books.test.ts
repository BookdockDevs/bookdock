...
describe('reading progress', () => {
  ...
  beforeAll(() => {
    db = createTestDb()
    userId = seedUser(db)
    bookId1 = seedBook(db, userId, { title: 'Book for create test' }).id
    bookId2 = seedBook(db, userId, { title: 'Book for update test' }).id
  })
  ...
  it('should create reading progress', () => {
    const progress = {
      ...
      userId,
      bookId: bookId1,
      cfi: '/6/2[Chapter1]!/4/2',
      chapter: null,
      percent: 50,
      updatedAt: Date.now(),
    }
    db.insert(schema.readingProgress).values(progress).run()
    const result = db.select().from(schema.readingProgress).where(eq(schema.readingProgress.id, progress.id)).get()
    expect(result).toBeDefined()
    expect(result!.percent).toBe(50)
  })

  it('should update reading progress', () => {
    const progressId = createId('prog')
    ...
    db.update(schema.readingProgress).set({ percent: 90, updatedAt: Date.now() }).where(eq(schema.readingProgress.id, progressId)).run()
    const result = db.select().from(schema.readingProgress).where(eq(schema.readingProgress.id, progressId)).get()
