...
// overshoot the text into the blank margins and sit too low.
...
const setupPanningEvents = (doc) => {
    if (doc._docEventsInitialized) return
    doc._docEventsInitialized = true
...
// high-water limit (~2 GB). Crash logs on low-memory devices show the
...
const render = async (page, doc, zoom, pageColors) => {
    ...
    // memory budget, see getRenderDpr) but lay the whole DOM out
...
// schemes serving these reads through range requests (such as Android's
// `shouldInterceptRequest` or native bridges) have no such limit, so
// firing every request at once floods the native handler and exhausts the
// WebView's heap, crashing on 50 MB+ PDFs. Throttle here.
