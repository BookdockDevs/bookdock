...
export class View extends HTMLElement {
  ...
  #onRelocate({ reason, range, index, fraction, size }) {
    ...
    if (cfi && (!this.#lastCfi || cfi !== this.#lastCfi
      // also re-emit when the fraction recovers from a transient NaN (section
      // reload with zero viewSize) so UI progress catches up without a cfi change
      || (Number.isFinite(this.lastLocation.fraction) && !Number.isFinite(this.#lastEmittedFraction)))) {
      this.#lastCfi = cfi
      this.#lastEmittedFraction = this.lastLocation.fraction
