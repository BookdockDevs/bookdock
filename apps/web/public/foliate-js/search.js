...
// regex mode: same offset-mapping as simpleSearch, but the needle is a
// user-supplied RegExp; empty matches are skipped so `.*` can't livelock
const regexSearch = function* (strs, query, options = {}) {
    let re
    try {
        re = new RegExp(query, options.sensitivity === 'variant' || options.sensitivity === 'case' ? 'gu' : 'giu')
    } catch {
        return // invalid pattern: no results, don't crash the whole search
    }
    const haystack = strs.join('')
    const starts = [0]
    for (const s of strs) starts.push(starts[starts.length - 1] + s.length)
    const locate = pos => {
        let i = 0
        while (i < strs.length - 1 && starts[i + 1] <= pos) i++
        return { index: i, offset: pos - starts[i] }
    }
    for (const m of haystack.matchAll(re)) {
        if (!m[0]) continue
        const start = locate(m.index)
        const end = locate(m.index + m[0].length)
        const range = {
            startIndex: start.index, startOffset: start.offset,
            endIndex: end.index, endOffset: end.offset,
        }
        yield { range, excerpt: makeExcerpt(strs, range) }
    }
}
...
export const search = (strs, query, options) => {
    ...
    if (options.regex) return regexSearch(strs, query, options)
...
export const searchMatcher = (textWalker, opts) => {
    const { defalutLocale, matchCase, matchDiacritics, matchWholeWords, matchRegex } = opts
