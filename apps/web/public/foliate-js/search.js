...
// Regex search mode. Runs a JS RegExp over the joined text and
...
// Nearby-words search mode: matches places where all of the
...
export const search = (strs, query, options) => {
    const { mode, regex } = options
    if (mode === 'regex' || regex) return regexSearch(strs, query, options)
