...
export class SectionProgress {
    ...
    getProgress(index, fractionInSection, pageFraction = 0) {
        ...
        return {
            ...
            // `fraction` includes the visible page tail in paginated flow.
            // Keep the viewport start separately so a seek can use the same
            // coordinate that the progress strip displays.
            startFraction: sizeTotal > 0 ? size / sizeTotal : 0,
            section: {
                current: index,
                total: sizes.length,
            },
            ...
            time: {
                ...
                total: sizePerTimeUnit > 0 ? remainingTotal / sizePerTimeUnit : 0,
            },
        }
    }
    ...
    getSection(fraction) {
        if (this.sizeTotal <= 0) return [0, 0]
