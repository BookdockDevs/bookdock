...
export default function AiBrandIcon({ name, model, provider, className = 'h-8 w-8', selected = false }: AiBrandIconProps) {
  ...
  return (
    <span
      ...
      title={label}
    >
      {asset ? <img src={asset} alt="" aria-hidden="true" decoding="async" className={`h-[62%] w-[62%] object-contain ${adaptive ? 'dark:invert' : ''}`} /> : <span className="text-[0.58em] font-semibold text-stone-500 dark:text-stone-300">{fallback}</span>}
    </span>
  )
}
