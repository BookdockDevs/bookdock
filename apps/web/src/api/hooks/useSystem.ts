 ...
 */
export function useSystemUpdateStatus(tracking = true) {
  return useQuery({
    ...
    retry: false,
    staleTime: 0,
    refetchInterval: (query) => query.state.data?.data.outcome === 'active' ? UPDATE_POLL_MS : false,
  })
}

export function useCancelSystemUpdate() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (progressId: string) => apiDelete<{ data: UpdateStatusRes }>(`/system/update/${encodeURIComponent(progressId)}`),
